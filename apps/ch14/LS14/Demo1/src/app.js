var HelloWorldLayer = cc.Layer.extend({
    ...
    ctor:function () {
        ...
        cc.log('tileMap 的 type 属性：' + tileMap.getProperty('type'));

        var tile1Properties = tileMap.getPropertiesForGID(1); // GID 与瓦片 id 差 1， GID = id + 1
        for (var i in tile1Properties) {
            cc.log('GID1 的属性' + i + ':' + tile1Properties[i]);
        }
        ...
        var tiles = bgLayer.getTiles(); // Tile 数组
        var tileAt = bgLayer.getTileAt(cc.p(1, 1)); // tileMap 坐标系下得到对应的精灵
        tileAt.setColor(cc.color.BLUE);
        var tileGIDAt = bgLayer.getTileGIDAt(cc.p(6, 5)); // 这里指的是 TileMap 坐标系
        cc.log('tileGID = ' + tileGIDAt);

        var positionAt = bgLayer.getPositionAt(1, 1); // 瓦片坐标系 -> 像素的坐标系转换，x 轴一样，y 轴反向并差一片，重要
        ...
        var properties = tileMap.getPropertiesForGID(1); // GID 从 1 开始
