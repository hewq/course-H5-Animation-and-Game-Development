"use strict";
cc._RF.push(module, '43c01ZihilCVbr527LIzQ4f', 'GAFAsset');
// frameworks/cocos2d-html5/external/gaf/Library/GAFAsset.js
...
gaf.Asset = cc.Class.extend({
     ...
     * @return {bool}
     */
    initWithGAFFile: function initWithGAFFile(filePath, textureLoadDelegate) {
        var self = this;
     ...
     * @return {bool}
     */
    initWithGAFBundle: function initWithGAFBundle(zipFilePath, entryFile, delegate) {
     ...
     * @param {String} name
     */
    setRootTimelineWithName: function setRootTimelineWithName(name) {
    ...
    isAssetVersionPlayable: function isAssetVersionPlayable() {
        return true;
    },
     ...
     * @returns {number}
     */
    desiredAtlasScale: function desiredAtlasScale() {
     ...
     * @param scale
     */
    setDesiredAtlasScale: function setDesiredAtlasScale(desiredAtlasScale) {
        this._desiredAtlasScale = desiredAtlasScale;
        for (var currentScale in this._atlasScales) {
            if (this._atlasScales.hasOwnProperty(currentScale)) {
                if (this._usedAtlasScale === 0 || Math.abs(this._usedAtlasScale - desiredAtlasScale) > Math.abs(currentScale - desiredAtlasScale)) {
     ...
     * @return {gaf.Object}
     */
    createObjectAndRun: function createObjectAndRun(looped) {
     ...
     */
    setTextureLoadDelegate: function setTextureLoadDelegate(delegate) {
        debugger;
    },
     ...
     * @return {uint}
     */
    getSceneFps: function getSceneFps() {
     ...
     * @return {uint}
     */
    getSceneHeight: function getSceneHeight() {
        debugger;
    },
     ...
     * @return {cc.color4b}
     */
    getSceneColor: function getSceneColor() {
        debugger;
    },
     ...
     * @param {uint} fps
     */
    setSceneFps: function setSceneFps(fps) {
        this._sceneFps = fps;
    },
     ...
     * @param {uint} width
     */
    setSceneWidth: function setSceneWidth(width) {
        debugger;
    },
     ...
     */
    setSceneColor: function setSceneColor(color4B) {
        debugger;
    },
    ...
    getGAFFileName: function getGAFFileName() {
    ...
    ctor: function ctor() {
        ...
        if (arguments.length > 0) this.initWithGAFFile.apply(this, arguments);
    },

    _getProtos: function _getProtos() {
        return this._protos;
    },

    _setRootTimeline: function _setRootTimeline(timeLine) {
        this._rootTimeLine = timeLine;
        this._header.pivot = timeLine.getPivot();
        this._header.frameSize = timeLine.getRect();
    },

    _setHeader: function _setHeader(gafHeader) {
    ...
    _getMajorVerison: function _getMajorVerison() {
        return this._header.versionMajor;
    },

    _init: function _init(gafData) {
        var self = this;
    ...
    _pushTimeLine: function _pushTimeLine(timeLine) {
    ...
    _instantiateGaf: function _instantiateGaf() {
        var root = null;
        root = this._rootTimeLine._gafConstruct();
        return root;
    },

    _onAtlasLoaded: function _onAtlasLoaded(id, atlas) {
        this._atlases[id] = atlas;
        delete this._atlasesToLoad[id];
        if (Object.keys(this._atlasesToLoad).length === 0) {
            this._onLoadTasks.forEach(function (fn) {
                fn();
            });
    ...
    _getSearchPaths: function _getSearchPaths(imageUrl) {
