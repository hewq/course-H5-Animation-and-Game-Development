"use strict";
cc._RF.push(module, '748c6qjLJlHp6EW7/cfFTAt', 'gaf_viewer');
// frameworks/cocos2d-html5/external/gaf/gaf_viewer.js
...
(_module || {}).exports = renderjson = function () {
    var themetext = function themetext() /* [class, text]+ */{
        var spans = [];
    ...
    var append = function append() /* el, ... */{
        ...
        for (var a = 0; a < arguments.length; a++) {
            if (arguments[a].constructor == Array) append.apply(this, [el].concat(arguments[a]));else el.appendChild(arguments[a]);
        }return el;
    };
    var prepend = function prepend(el, child) {
    ...
    var span = function span(classname) {
    ...
    function _renderjson(json, indent, dont_indent, show_level, sort_objects) {
        ...
        if ((typeof json === 'undefined' ? 'undefined' : _typeof(json)) != "object") // Strings, numbers and bools
            return themetext(null, my_indent, typeof json === 'undefined' ? 'undefined' : _typeof(json), JSON.stringify(json));

        var disclosure = function disclosure(open, close, type, builder) {
            var content;
            var empty = span(type);
            ...
            append(empty, A(renderjson.show, "disclosure", show), color_rect, themetext(type + " syntax", open), A(placeholder, null, show), themetext(type + " syntax", close));
        ...
        if (json.constructor == Array) {
            ...
            return disclosure("[", "]", "array", function () {
                ...
                for (var i = 0; i < json.length; i++) {
                    append(as, _renderjson(json[i], indent + "    ", false, show_level - 1, sort_objects), i != json.length - 1 ? themetext("syntax", ",") : [], text("\n"));
        ...
        return disclosure("{", "}", "object", function () {
            ...
            for (var i in keys) {
                var k = keys[i];
                append(os, themetext(null, indent + "    ", "key", '"' + k + '"', "object syntax", ': '), _renderjson(json[k], indent + "    ", true, show_level - 1, sort_objects), k != last ? themetext("syntax", ",") : [], text("\n"));
            }
