"use strict";
cc._RF.push(module, '9f2fcURRtVM6atto/iHs8Qg', 'GAFAtlasLoader');
// frameworks/cocos2d-html5/external/gaf/Library/GAFAtlasLoader.js
...
{
    return function () {
        return atlas && typeof atlas !== "string" && atlas.isLoaded();
    };
};

gaf._AtlasLoader.load = function (path, success, fail) {
    cc.textureCache.addImage(path, function (atlas) {
        gaf._AtlasLoader.execute(gaf._AtlasLoader.checkAtlas(atlas), function () {
            success(atlas);
        }, fail);
    });
};
...
gaf._AtlasLoader.loadFront = function (arr, success, fail) {
    ...
    return function () {
        if (arr.length > 0) {
            gaf._AtlasLoader.load(arr[0], success, gaf._AtlasLoader.loadFront(arr.slice(1), success, fail));
        } else fail();
    };
};
