"use strict";
cc._RF.push(module, '6bf09EaqJJG5bQiyfOucbbL', 'GAFTimeLine');
// frameworks/cocos2d-html5/external/gaf/Library/GAFTimeLine.js
...
gaf.TimeLine = gaf.Object.extend({
    ...
    setLooped: function setLooped(looped, recursively) {
    ...
    playSequence: function playSequence(name, looped) {
    ...
    ctor: function ctor(gafTimeLineProto, scale) {
        this._super(scale);
        this._objects = [];
    ...
    _processAnimations: function _processAnimations(dt) {
        this._timeDelta += dt;
    ...
    _step: function _step() {
        ...
        if (this._isCurrentFrameLastInSequence()) {
            if (this._isLooped) {
                if (this._animationStartedNextLoopDelegate) this._animationStartedNextLoopDelegate(this);
            } else {
                this.setAnimationRunning(false, false);
                if (this._animationFinishedPlayDelegate) this._animationFinishedPlayDelegate(this);
            }
        }
        this._processAnimation();
    ...
    _isCurrentFrameLastInSequence: function _isCurrentFrameLastInSequence() {
        if (this._isReversed) return this._currentFrame == this._currentSequenceStart;
        return this._currentFrame == this._currentSequenceEnd - 1;
    },

    _nextFrame: function _nextFrame() {
        if (this._isCurrentFrameLastInSequence()) {
            if (!this._isLooped) return this._currentFrame;

            if (this._isReversed) return this._currentSequenceEnd - 1;else return this._currentSequenceStart;
        }
    ...
    _realizeFrame: function _realizeFrame(out, frameIndex) {
        var self = this;
    ...
    setAnimationRunning: function setAnimationRunning(value, recursively) {
    ...
    _getSequenceByLastFrame: function _getSequenceByLastFrame() {
    ...
    _getSequenceByFirstFrame: function _getSequenceByFirstFrame() {
        var sequences = this._gafproto.getSequences();
        for (var item in sequences) {
            if (sequences.hasOwnProperty(item)) {
