var MainLayer = cc.Layer.extend({
	...
	ctor: function () {
		...
		// 控制按钮
		var leftItem = new cc.MenuItemFont('left', function () {
			if (p1.x > 100) {
				p1.x -= 50;
			}
		}, this);

		var rightItem = new cc.MenuItemFont('right', function () {
		...
		// 分数标签
		var scoreLabel = new cc.LabelTTF('0 mile', '', size.width / 10);
	...
	bgCallBack: function (dt) {
		for (var i in this.arr) {
			...
			this.arr[i].y -= 2;
			this.score += 1;
			this.scoreLabel.setString(this.score + 'mile');
		}
	},
